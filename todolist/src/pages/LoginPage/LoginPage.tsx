...
function LoginPage() {
    ...
    const [statusSignUp, setStatusSignUp] = useState({opacity: 0, content: '', color: ''})
    ...
    useEffect(() => {
        const getLogo = async() => {
            const logo = await storeRef.ref('todo-logo.webp').getDownloadURL()
                .then(url => url)
    ...
    const handleSignInButton = async() => {
        const statusAuthSignIn: StatusAuth = await signUserIn(loginEmail, loginPassword)
        console.log(statusAuthSignIn)
        if (statusAuthSignIn.verified && statusAuthSignIn.userType === 'user') {
            ...
            setStatusLogin({opacity: 1, content: `E-mail or password doesn't exist`})
        } 
    } 
    return (
        <ContainerForm>
            <ContentForm>
                <img src={logoUrlState} style={{width: "100px", height: "100px"}} />
                <h1>Welcome to your ToDos!</h1>
                <h2>Log In to create, edit and much more!</h2>
                <p style=
                    {
                        {opacity: statusLogin.opacity, 
                        color:"#F00"
                        }
                    }
                >
